/* ============================================================================
   ...
   Lenguaje visual: web pública aprobada (web.html) + Manual de identidad Beme.
   ========================================================================== */
(function (global) {
'use strict';
...
/* ---------------------------------------------------------------------------
   ...
   Se inyecta con BemeArtes.injectCSS(). Todas las medidas están en píxeles
...
@font-face{
  ...
  src:local('Just Sans'),
      url('assets/brand/fonts/JustSans-Regular.woff2') format('woff2'),
      url('assets/brand/fonts/PlusJakartaSans-Variable.woff2') format('woff2-variations'),
      url('assets/brand/fonts/PlusJakartaSans-Variable.woff2') format('woff2');
  font-weight:200 800;font-style:normal;font-display:swap;
}
.a{
  --bg:#0f0f11; --bg2:#17171a; --ink:#f4f4f4; --gray:#9b9ba4;
...
/* --- Fondos --- */
.a[data-bg="claro"]{--bg:#f4f4f4;--ink:#1c1c1c;--gray:#5c5c66;--stroke:rgba(28,28,28,.42)}
.a[data-bg="vibrante"]{--bg:#9414e0;--stroke:rgba(255,255,255,.6)}
.a .aura{position:absolute;border-radius:50%;filter:blur(150px);pointer-events:none}
.a[data-bg="claro"] .aura{opacity:.28;filter:blur(170px)}
.a .grain{position:absolute;inset:0;pointer-events:none;opacity:.5;
...
.a .fill-brand{position:absolute;inset:0;background:var(--g-brand)}
.a .fill-deep{position:absolute;inset:0;background:linear-gradient(150deg,#440b5d 0%,#b2005d 55%,#d90e86 100%)}

/* --- Tipografía --- */
.a .disp{
  text-transform:uppercase;font-weight:800;line-height:.88;letter-spacing:-.035em;
...
.a .disp .pinkdot{color:var(--pink);-webkit-text-fill-color:var(--pink)}
.a .serif{text-transform:none;font-weight:700;letter-spacing:-.03em;line-height:1.02}

.a .lbl{display:flex;align-items:center;gap:20px;color:var(--pink);
  font-size:21px;font-weight:700;letter-spacing:.22em;text-transform:uppercase}
.a .lbl .dash{display:block;width:64px;height:2px;background:var(--pink);flex:0 0 64px}
...
.a .mini{font-size:20px;font-weight:600;letter-spacing:.16em;text-transform:uppercase;color:var(--gray)}

/* --- Componentes --- */
.a .pill{display:inline-flex;align-items:center;padding:12px 26px;border-radius:999px;
  background:var(--g-vibrant);color:#fff;font-size:19px;font-weight:700;
  letter-spacing:.13em;text-transform:uppercase;white-space:nowrap}
.a .pill.ghost{background:transparent;border:2px solid rgba(244,244,244,.32);color:var(--ink)}
.a[data-bg="claro"] .pill.ghost{border-color:rgba(28,28,28,.24)}

.a .btn{display:inline-flex;align-items:center;gap:20px;padding:28px 48px;border-radius:999px;
  background:var(--g-vibrant);color:#fff;font-size:28px;font-weight:700;letter-spacing:-.01em}
.a .btn.light{background:#f4f4f4;color:#0f0f11}
...
.a .acard{border-radius:34px;overflow:hidden;position:relative;background:#1f1f24}
.a[data-bg="claro"] .acard{background:#e6e6e8}

.a .vnum{position:absolute;writing-mode:vertical-rl;transform:rotate(180deg);
  font-size:24px;font-weight:700;letter-spacing:.28em;color:var(--pink)}

.a .logo{position:absolute;display:block;object-fit:contain}
.a .dots{position:absolute;display:flex;gap:15px}
.a .dots i{display:block;width:17px;height:17px;border-radius:50%;background:currentColor}

.a .scrim{position:absolute;left:0;right:0;pointer-events:none}
.a .scrim.bot{background:linear-gradient(to top,#0f0f11 4%,rgba(15,15,17,.82) 42%,rgba(15,15,17,0) 100%)}
.a .scrim.top{background:linear-gradient(to bottom,rgba(15,15,17,.85) 0%,rgba(15,15,17,0) 100%)}
.a .scrim.mag{background:linear-gradient(to top,rgba(178,0,93,.95) 2%,rgba(148,20,224,.45) 48%,rgba(15,15,17,0) 100%)}

/* --- Slots de imagen --- */
.a .slot{position:absolute;background-size:cover;background-position:center;
  background-repeat:no-repeat;background-color:#1f1f24;cursor:pointer}
...
.a .slot .ph{position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;
  ...
  border:4px dashed rgba(237,86,143,.45);color:var(--pink);background:rgba(31,31,36,.9);
...
.a .safe{position:absolute;border:3px dashed rgba(237,86,143,.7);pointer-events:none;
  display:flex;align-items:flex-start;justify-content:flex-end}
.a .safe span{margin:10px;padding:7px 14px;border-radius:8px;background:rgba(15,15,17,.82);
  color:#ed568f;font-size:18px;font-weight:700;letter-spacing:.1em;text-transform:uppercase}

/* --- Edición --- */
.a [data-field]{outline:0}
.a [data-field]:hover{box-shadow:0 0 0 2px rgba(148,20,224,.4);border-radius:6px}
.a [data-field]:focus{box-shadow:0 0 0 3px rgba(237,86,143,.9);border-radius:6px}
...
const CHECK = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" style="width:34px;height:34px;flex:0 0 34px;margin-top:6px"><polyline points="20 6 9 17 4 12"/></svg>';
...
function slot(key, style, cls, label){
  const has = ''; // el editor le agrega .has cuando hay imagen
  return `<div class="slot ${cls||''} ${has}" data-img="${key}" style="${style||''}">
    <div class="ph">${PHIMG}${label ? label.replace(/\n/g,'<br>') : 'Imagen'}</div></div>`;
}
/** Auras de fondo (dos manchas radiales, como en la web) */
function auras(preset){
  // Halos sutiles: el negro sigue mandando, como en la web pública.
  const p = {
  ...
  return p[preset] || p.esquinas;
}
/** Cabecera estándar: logo + tres puntos del manual */
function header(y, opts){
  const o = opts || {};
  const w = o.logoW || 116;
  const src = o.dark ? LOGO.color : LOGO.blanco;
  return `<img class="logo" src="${src}" alt="Beme" style="top:${y}px;left:${o.x||80}px;width:${w}px">
          <div class="dots" style="top:${y + Math.round(w*0.28)}px;right:${o.x||80}px;color:${o.dotColor || (o.dark ? '#b2005d' : '#f4f4f4')}"><i></i><i></i><i></i></div>`;
}
...
/* ---------------------------------------------------------------------------
   4. Plantillas
   Cada una: { id, nombre, grupo, w, h, render(d) → HTML interno del artboard }
--------------------------------------------------------------------------- */
const TEMPLATES = [

/* ═══════════════════ FEED 4:5 · 1080×1350 ═══════════════════ */
{
  id:'talento-destacado', nombre:'Talento destacado', grupo:'Feed 4:5', clase:'f45', bg:'oscuro',
  render(d){
    const t = d.talentos[0] || {};
    const s = t.seguidores || {};
    const partes = String(t.nombre||'').trim().split(/\s+/);
    const nom1 = partes[0] || '';
    const nom2 = partes.slice(1).join(' ');
    const mets = [
    ...
    return `
      ${slot('talento0.foto','inset:0', '', 'Foto vertical del talento\n1080×1350')}
      <div class="scrim top" style="top:0;height:400px"></div>
      <div class="scrim bot" style="bottom:0;height:880px"></div>
      ${header(80)}
      <div style="position:absolute;left:80px;right:80px;bottom:86px">
        ${f('label','<span class="dash"></span>Roster Beme','lbl')}
        <div class="disp fit" style="font-size:132px;margin-top:34px">
          ${fs('talento0.nombre1', esc(nom1))}<br>
          ${fs('talento0.nombre2', `<span class="out">${esc(nom2)}</span>`)}
        </div>
        ${f('talento0.handle', esc(t.handle||''),'copy','margin-top:26px;font-size:32px;color:var(--ink);font-weight:500')}
        <div style="display:flex;gap:14px;flex-wrap:wrap;margin-top:30px">
          ${(t.categorias||[]).slice(0,3).map((c,i)=>
            `<span class="pill" data-field="talento0.cat${i}" contenteditable="true">${esc(c)}</span>`).join('')}
        </div>
        <div style="display:flex;gap:76px;margin-top:52px">
          ${mets.map((m,i)=>`<div>
            ${f('met'+i+'.n', esc(m.n),'num','font-size:92px')}
...
{
  id:'caso-exito', nombre:'Resultados de campaña', grupo:'Feed 4:5', clase:'f45', bg:'oscuro',
  render(d){
    ...
    return `
      ...
      <div style="position:absolute;left:80px;right:80px;top:250px">
        ...
        <div class="disp fit" style="font-size:104px;margin-top:34px">
          ${fs('campana1', esc(a))}${b?'<br>':''}${b?fs('campana2',`<span class="out">${esc(b)}</span>`):''}
        </div>
      </div>
      ${slot('marcaLogo','left:80px;top:610px;width:300px;height:110px;background-position:left center','contain','Logo de la marca')}
      <div style="position:absolute;left:80px;right:80px;top:790px;display:flex;gap:64px;flex-wrap:wrap">
        ${(d.metricas||[]).slice(0,3).map((m,i)=>`<div>
          ${f('metrica'+i+'.n', esc(m.n),'num')}
          ${f('metrica'+i+'.l', esc(m.l),'numl')}
        </div>`).join('')}
      </div>
      ${f('subtitulo', esc(d.subtitulo||''),'copy','position:absolute;left:80px;right:150px;top:1030px')}
      <div style="position:absolute;left:80px;bottom:90px;display:flex;align-items:center">
        ${(d.talentos||[]).slice(0,4).map((t,i)=>
          `<div class="slot round" data-img="talento${i}.foto" style="position:relative;width:104px;height:104px;
            border:5px solid #0f0f11;margin-left:${i?'-30px':'0'};z-index:${9-i}">
            <div class="ph" style="font-size:0;border-width:2px"></div></div>`).join('')}
        ${f('creditos', creditos(d.talentos),'copy','margin-left:30px;font-size:24px;line-height:1.35')}
      </div>`;
  }
},
{
  id:'frase', nombre:'Frase / statement', grupo:'Feed 4:5', clase:'f45', bg:'oscuro',
  render(d){
    // el punto final lo pone la pieza en rosa, así que se lo saco al texto
    const [a,b] = splitHalf(String(d.frase||'').trim().replace(/\.$/,''));
    return `
      ${auras('abajo')}
      ${header(80)}
      <div style="position:absolute;left:80px;right:80px;top:400px">
        <div class="disp serif" style="font-size:96px;line-height:1.04">
          ${fs('frase1', esc(a))} ${fs('frase2', `<span class="grad">${esc(b)}</span>`)}<span class="pinkdot">.</span>
        </div>
      </div>
      <div style="position:absolute;left:80px;bottom:110px">
        ${f('firma','Beme Agency · Marketing de influencers','mini')}
      </div>`;
  }
},
{
  id:'carrusel-portada', nombre:'Portada de carrusel', grupo:'Feed 4:5', clase:'f45', bg:'oscuro',
  render(d){
    const [a,b] = splitHalf(d.titulo);
    return `
      ${auras('esquinas')}
      ${header(80)}
      <div class="acard" style="position:absolute;right:64px;top:190px;width:400px;height:520px;transform:rotate(5deg)">
        ${slot('portada.img','inset:0','','Foto o mockup')}
      </div>
      <div style="position:absolute;left:80px;right:80px;top:760px">
        ${f('label','<span class="dash"></span>Guía para marcas','lbl')}
        <div class="disp fit" style="font-size:112px;margin-top:32px">
          ${fs('titulo1', esc(a))}<br>${fs('titulo2', `<span class="grad">${esc(b)}</span>`)}
        </div>
        ${f('subtitulo', esc(d.subtitulo||''),'copy','margin-top:34px;max-width:820px')}
      </div>
      <div style="position:absolute;left:80px;bottom:88px">
        <span class="btn">${fs('cta_deslizar','Deslizá')}${ARROW}</span>
      </div>`;
  }
},
{
  id:'carrusel-slide', nombre:'Slide de carrusel', grupo:'Feed 4:5', clase:'f45', bg:'oscuro',
  render(d){
    const s = (d.slides && d.slides[0]) || {};
    return `
      ${auras('esquinas')}
      ${f('slide0.n', esc(s.n||'01'),'vnum','left:88px;top:300px;height:200px;font-size:30px')}
      ${header(80,{x:80})}
      <div class="acard" style="position:absolute;left:170px;right:80px;top:230px;height:400px">
        ${slot('slide0.img','inset:0','','Imagen del slide')}
      </div>
      <div style="position:absolute;left:170px;right:80px;top:700px">
        ${f('slide0.titulo', esc(s.titulo||''),'disp','font-size:74px')}
        <div style="width:110px;height:5px;background:var(--pink);border-radius:3px;margin:38px 0"></div>
        ${f('slide0.texto', esc(s.texto||''),'copy')}
      </div>
      ${f('slide0.pie','Beme Agency','mini','position:absolute;left:170px;bottom:80px')}`;
  }
},
{
  id:'marcas', nombre:'Marcas que confían', grupo:'Feed 4:5', clase:'f45', bg:'oscuro',
  render(d){
    ...
    return `
      ...
      <div style="position:absolute;left:80px;right:80px;top:250px">
        ...
        <div class="disp fit" style="font-size:100px;margin-top:32px">
          ${fs('marcas1','Marcas que')}<br>${fs('marcas2','<span class="grad">confían</span> en Beme')}
        </div>
      </div>
      <div style="position:absolute;left:80px;right:80px;top:660px;display:grid;grid-template-columns:repeat(3,1fr);gap:22px">
        ${logos.slice(0,9).map((src,i)=>
          `<div class="acard" style="height:150px;border-radius:24px">
             <div class="slot contain ${src?'has':''}" data-img="marcasLogos.${i}"
                  style="inset:22px;${src?`background-image:url('${src}')`:''}">
               <div class="ph" style="font-size:16px;border-width:2px;border-radius:16px">Logo</div>
             </div>
           </div>`).join('')}
      </div>
      ${f('pie','+ 40 marcas activas en LATAM, EE.UU. y Europa','mini','position:absolute;left:80px;bottom:80px')}`;
  }
},
{
  id:'convocatoria', nombre:'Convocatoria de creadores', grupo:'Feed 4:5', clase:'f45', bg:'vibrante',
  render(d){
    return `
      <div class="fill-deep"></div>
      ${header(80)}
      <div style="position:absolute;left:80px;right:80px;top:300px">
        ${f('label','<span class="dash" style="background:#fff"></span>Casting abierto','lbl','color:#fff')}
        <div class="disp fit" style="font-size:126px;margin-top:34px">
          ${fs('conv1','Buscamos')}<br>${fs('conv2','<span class="out">creadores</span>')}
        </div>
        ${f('conv_sub','Sumate al roster de Beme y trabajá con marcas internacionales.','copy','margin-top:34px;max-width:840px;color:rgba(255,255,255,.9)')}
      </div>
      <div style="position:absolute;left:80px;right:80px;top:860px;display:flex;flex-direction:column;gap:28px">
        ${(d.bullets||[]).slice(0,3).map((b,i)=>
          `<div style="display:flex;gap:22px;align-items:flex-start;color:#fff">${CHECK}
            ${f('bullet'+i, esc(b),'copy','color:#fff')}</div>`).join('')}
      </div>
      <div style="position:absolute;left:80px;bottom:90px">
        <span class="btn light">${fs('cta','Postulate en el link de la bio')}${ARROW}</span>
      </div>`;
  }
},
...
{
  id:'dato', nombre:'Dato del mercado', grupo:'Cuadrado 1:1', clase:'sq', bg:'oscuro',
  render(d){
    const x = d.dato || {};
    return `
      ${auras('esquinas')}
      ...
      <div style="position:absolute;left:80px;right:80px;top:300px">
        ...
        ${f('dato.n', esc(x.n||''),'num','font-size:250px;margin-top:20px')}
        ${f('dato.txt', esc(x.txt||''),'disp serif','font-size:54px;margin-top:36px;font-weight:400;letter-spacing:-.02em;line-height:1.18')}
      </div>
      ${f('dato.fuente', esc(x.fuente||''),'mini','position:absolute;left:80px;bottom:80px')}`;
  }
},
{
  ...
  render(d){
    ...
    return `
      ...
      <div style="position:absolute;left:80px;right:80px;bottom:80px">
        ...
        <div class="disp fit" style="font-size:110px;margin-top:28px">
          ${fs('marca1', esc(a))}${b?`<br>${fs('marca2',`<span class="out" style="-webkit-text-stroke-color:rgba(255,255,255,.75)">${esc(b)}</span>`)}`:''}
        </div>
        ${f('campana.talentos', (d.talentos||[]).slice(0,3).map(t=>esc(t.handle)).join(' · '),'copy','margin-top:24px;color:#fff;font-size:26px')}
        <div style="display:flex;gap:14px;flex-wrap:wrap;margin-top:28px">
          ${f('campana.periodo', esc(d.periodo||''),'pill').replace('<div','<span').replace('</div>','</span>')}
          ${f('campana.hashtag', esc(d.hashtag||''),'pill ghost','color:#fff').replace('<div','<span').replace('</div>','</span>')}
        </div>
      </div>`;
  }
},
...
{
  id:'story-talento', nombre:'Story de talento', grupo:'Stories 9:16', clase:'st', bg:'oscuro',
  render(d){
    const t = d.talentos[0] || {};
    const s = t.seguidores || {};
    const partes = String(t.nombre||'').trim().split(/\s+/);
    ...
    return `
      ${slot('talento0.foto','inset:0','','Foto vertical\n1080×1920')}
      <div class="scrim top" style="top:0;height:560px"></div>
      <div class="scrim bot" style="bottom:0;height:1060px"></div>
      ${header(300)}
      <div style="position:absolute;left:80px;right:80px;bottom:440px">
        ${f('label','<span class="dash"></span>Nuevo en el roster','lbl')}
        <div class="disp fit" style="font-size:142px;margin-top:34px">
          ${fs('talento0.nombre1', esc(partes[0]||''))}<br>
          ${fs('talento0.nombre2', `<span class="out">${esc(partes.slice(1).join(' '))}</span>`)}
        </div>
        ${f('talento0.handle', esc(t.handle||''),'copy','margin-top:26px;font-size:34px;color:var(--ink)')}
        <div style="display:flex;gap:70px;margin-top:48px">
          ${mets.map((m,i)=>`<div>${f('smet'+i+'.n', esc(m.n),'num','font-size:88px')}${f('smet'+i+'.l', esc(m.l),'numl')}</div>`).join('')}
        </div>
        <div style="margin-top:56px"><span class="btn">${fs('cta_perfil','Ver perfil completo')}${ARROW}</span></div>
      </div>
...
{
  id:'story-anuncio', nombre:'Story de anuncio', grupo:'Stories 9:16', clase:'st', bg:'vibrante',
  render(d){
    const [a,b] = splitHalf(d.titulo);
    return `
      <div class="fill-vibrante"></div>
      <div class="aura" style="left:-200px;bottom:200px;width:900px;height:900px;background:rgba(178,0,93,.75)"></div>
      ${header(300,{logoW:130})}
      <div style="position:absolute;left:80px;right:80px;top:680px">
        ${f('label','<span class="dash" style="background:#fff"></span>Novedad','lbl','color:#fff')}
        <div class="disp fit" style="font-size:130px;margin-top:34px">
          ${fs('stitulo1', esc(a))}<br>${fs('stitulo2', `<span class="out">${esc(b)}</span>`)}
        </div>
        ${f('ssub', esc(d.subtitulo||''),'copy','margin-top:38px;color:rgba(255,255,255,.9)')}
      </div>
      ...
      <div style="position:absolute;left:80px;bottom:296px"><span class="btn light">${fs('cta', esc(d.cta||''))}${ARROW}</span></div>
      <div class="safe" style="left:0;right:0;top:0;height:250px;border-width:0 0 3px"><span>UI de Instagram</span></div>
      <div class="safe" style="left:0;right:0;bottom:0;height:250px;border-width:3px 0 0"><span>UI de Instagram</span></div>`;
  }
},
  ...
  render(d){
    ...
    return `
      ...
      <div style="position:absolute;left:80px;right:80px;top:780px">
        ...
        <div class="disp fit" style="font-size:136px;margin-top:38px">
          ${fs('rtitulo1', esc(a))}<br>${fs('rtitulo2', `<span class="grad">${esc(b)}</span>`)}
        </div>
        ${f('reel.sub','En 40 segundos','copy','margin-top:32px;font-size:32px')}
      </div>
      <img class="logo" src="${LOGO.blanco}" alt="Beme" style="bottom:420px;left:80px;width:118px">
...
function getPath(obj, path){
  if (!path) return '';
  // atajo: talentoN.campo → talentos[N].campo
