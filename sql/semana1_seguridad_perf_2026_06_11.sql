...
-- Idempotente: dropea cada policy (nombre viejo Y nuevo) antes de crearla,
-- así se puede re-ejecutar sin "already exists".
-- ══════════════════════════════════════════════════════════════════
...
-- A2. talentos: lectura para autenticados, escritura solo interna.
DROP POLICY IF EXISTS talentos_auth_all ON talentos;
DROP POLICY IF EXISTS talentos_auth_read ON talentos;
DROP POLICY IF EXISTS talentos_internal_insert ON talentos;
DROP POLICY IF EXISTS talentos_internal_update ON talentos;
DROP POLICY IF EXISTS talentos_internal_delete ON talentos;
...
-- A3. rosters: escritura solo interna (policies anon de links públicos quedan igual).
DROP POLICY IF EXISTS rosters_auth_all ON rosters;
DROP POLICY IF EXISTS rosters_auth_read ON rosters;
DROP POLICY IF EXISTS rosters_internal_write ON rosters;
DROP POLICY IF EXISTS rosters_internal_update ON rosters;
DROP POLICY IF EXISTS rosters_internal_delete ON rosters;
...
DROP POLICY IF EXISTS prospeccion_contactos_internal ON prospeccion_contactos;
...
DROP POLICY IF EXISTS prospeccion_templates_internal ON prospeccion_email_templates;
...
DROP POLICY IF EXISTS prospeccion_email_log_internal ON prospeccion_email_log;
...
DROP POLICY IF EXISTS prosp_seq_internal ON prospeccion_email_secuencias;
...
DROP POLICY IF EXISTS prosp_cola_internal ON prospeccion_email_cola;
...
-- └──────────────────────────────────────────────────────────────┘
CREATE INDEX IF NOT EXISTS idx_contenido_observaciones_contenido ON contenido_observaciones(contenido_id);
CREATE INDEX IF NOT EXISTS idx_contenido_scripts_contenido ON contenido_scripts(contenido_id);
CREATE INDEX IF NOT EXISTS idx_contenido_borradores_contenido ON contenido_borradores(contenido_id);
CREATE INDEX IF NOT EXISTS idx_contenido_historial_contenido ON contenido_historial(contenido_id);
CREATE INDEX IF NOT EXISTS idx_contenido_briefs_contenido ON contenido_briefs(contenido_id);
CREATE INDEX IF NOT EXISTS idx_contenido_estadisticas_contenido_created ON contenido_estadisticas(contenido_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_campanas_updated_at ON campanas(updated_at DESC NULLS LAST);
...
BEGIN
  ...
  ELSE
    RAISE WARNING 'Hay % numeros de contrato duplicados — limpiar antes del indice unico.', dup_count;
  END IF;
END $$;
